...
func DefaultErrorHandler(w http.ResponseWriter, r *http.Request, err error, result *ImplResponse) {
	if _, ok := err.(*ParsingError); ok {
		...
		EncodeJSONResponse(err.Error(), func(i int) *int { return &i }(http.StatusBadRequest), w) //nolint: errcheck
	} else if _, ok := err.(*RequiredError); ok {
		// Handle missing required errors
		EncodeJSONResponse(err.Error(), func(i int) *int { return &i }(http.StatusUnprocessableEntity), w) //nolint: errcheck
	} else {
